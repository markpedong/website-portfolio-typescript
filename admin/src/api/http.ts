...
const upload = async <T>(url: string, data): Promise<ApiResponse<{ data: T }>> => {
	...
	//prettier-ignore
	const response = await instance.post(`https://portfolio-client-ebnp.onrender.com${url}`, form, {
		headers: {
...
const post = async <T>(url: string, data = {}): Promise<ApiResponse<T>> =>
	instance.post(`https://portfolio-client-ebnp.onrender.com${url}`, data, {
		headers: {
...
const get = async <T>(url: string, data = {}): Promise<ApiResponse<{ data: T }>> =>
	instance.get(`https://portfolio-client-ebnp.onrender.com${url}${stringify(data) ? '?' + stringify(data) : ''}`, {
		headers: {
			token: getLocalStorage('token')
		}
	})
